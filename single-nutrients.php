<?php
/**
 * The template for displaying all nutrients single posts
 *
...
<?php
	...
	if ($thumbnail == '') {
		$thumbnail = get_stylesheet_directory_uri().'/assets/images/placeholder.png';
	}
	...
	function nutrients_field($header,$id) {
		global $post;
		$html = '';
		foreach ($header as $key => $title) {
			$html .= '<p class="d-flex nutrients-row" >
			<span class="left font-weight-bold">
					'.$title.'
			</span><span class="right">'.get_field($key, $id).'</span></p>';
		}
		$html .='</p>';
		return $html;
	}
...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="entry-content mt-0">
		...
		echo nutrients_field($header,$post->ID);
		wp_link_pages(
			array(
