...
function ajax_search_init(){
	wp_register_script('ajax-search-script', get_stylesheet_directory_uri() . '/assets/js/home-search.js', array('jquery') );
	wp_enqueue_script('ajax-search-script');
	wp_localize_script( 'ajax-search-script',
						'ajax_search_object',
						array(
...
add_action('init', 'ajax_search_init');


function result_header($data) {
	$html = '';
...
function search_result_body($header,$id) {
	global $post;
	$html = '<tr class="tb-row">';
	foreach ($header as $key => $title) {
		if ($key == 'link') {
			$html .= '
			<td class="tab-col">
					<a href="'.get_the_permalink($id).'">View</a>
			</td>';
		}
		else if ($key == 'title') {
			$html .= '
...
function customize_home_search(){
	...
	$header = array(
		...
		'protein'=>'Protein',
		'fat'=>'FAT',
		'soluable_fiber'=>'Soluable',
		'insoluable_fiber'=>'InSoluable',
		'calcium'=>'Calcium',
		'iron'=>'Iron',
		'potassium'=>'Potassium',
		'sodium'=>'Sodium',
	...
	if( $wp_query->have_posts() ):
	    ...
	    while( $wp_query->have_posts() ): $wp_query->the_post();
				$return['data'] .= search_result_body($header,$post->ID);
	    endwhile;
			$data['data'] .='</tbody>';
	endif;
	$return['data'] .= '</table>';
